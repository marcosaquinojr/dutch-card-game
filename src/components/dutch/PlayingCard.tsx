  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className="flex h-full w-full flex-col justify-between p-1.5 sm:p-2">
            {/* Top corner */}
            <div className={cn("flex items-center justify-between font-display font-bold leading-none", suitColor(card?.suit ?? ""))}>
              <span className="text-xs sm:text-sm">{card?.value ?? "?"}</span>
            ...
            <div className="flex items-center justify-between text-[10px] font-semibold text-white/70 leading-none">
              <span className="text-[8px] sm:text-[9px] font-mono opacity-60">
                {card?.points !== undefined ? `${card.points}pt` : ""}
              </span>
              <span className={cn("rotate-180 font-display text-xs sm:text-sm", suitColor(card?.suit ?? ""))}>
                {card?.value}
              </span>
            </div>
          </div>
